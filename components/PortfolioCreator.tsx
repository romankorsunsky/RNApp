...
import { Portfolio, PortfolioAddProp, PortfolioCreationRequest, PortfolioType } from "@/Types/Types";
import { useRouter } from "expo-router";
import { useContext, useState } from "react";
import { Text, TextInput, TouchableOpacity, View } from "react-native";
import ThemedButton from "./ThemedButton";

export default function PortfolioCreator({closeModal}:PortfolioAddProp){
    ...
    const addPortfolioRequest = async function(){
        ...
        try{
            ...
            if(response.status === 200){
                ...
                console.log(`[LOG] Added portfolio \n ${JSON.stringify(portfolio)}`);
                router.replace("/(tabs)/(secs)/portfolios");
            }
            else{
            ...
            if(e instanceof Error){
                console.log(`[ERROR] in addPortfolioRequest():` + e.message);
                router.replace("/(tabs)/(secs)/portfolios");
            }
        }
        finally{
    ...
    return(
        <View className="mt-2 w-full h-full bg-white pt-2 mt-20">
